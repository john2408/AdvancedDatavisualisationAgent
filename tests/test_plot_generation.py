...
class TestPlotGeneration(unittest.TestCase):
    ...
    def test_chart_type_enum_values(self):
        ...
        expected_chart_types = [
            ...
            'PIE', 'SCATTER', 'HISTOGRAM', 'BOX', 'HEATMAP', 'CASCADE'
        ]
    ...
    def test_create_chart_plan_yoy_electric_vehicles(self):
        """
        Test that create_chart_plan returns correct ChartPlan object for year-over-year electric vehicle growth trends.
        
        This test validates that when given:
        - A DataFrame with year-over-year data (year_report, total_ev_registrations)
        - A user query asking for year-over-year growth trends for electric vehicles
        
        The function returns a ChartPlan object matching the expected structure for CASCADE chart (as year-over-year comparisons are best shown with cascade/waterfall charts that show deltas).
        """
        
        # Load test data
        self.test_dataframe = self.load_test_dataframe_from_json(filename="yoy_electric_veh.json")
        
        # Load expected ChartPlan structure
        expected_plan_path = project_root / "tests" / "test_data" / "chart_plan_yoy_electric.json"
        with open(expected_plan_path, 'r') as f:
            expected_plan_dict = json.load(f)
        
        # Test input values
        user_query = "What are the year-over-year growth trends for electric vehicles (2023 vs 2024)?"
        
        # Validate test data structure
        self.assertIsInstance(self.test_dataframe, pd.DataFrame)
        self.assertFalse(self.test_dataframe.empty, "Test DataFrame should not be empty")
        self.assertIn('year_report', self.test_dataframe.columns, "DataFrame should contain 'year_report' column")
        self.assertIn('total_ev_registrations', self.test_dataframe.columns, "DataFrame should contain 'total_ev_registrations' column")
        ...
        print(f"✅ Test passed: create_chart_plan returned correct ChartPlan object for YoY electric vehicle trends")
        print(f"   Chart Type: {result_plan_dict['chart_type']}")
        print(f"   X Column: {result_plan_dict['x']}")
        print(f"   Y Columns: {result_plan_dict['y']}")
        print(f"   Color Column: {result_plan_dict['color']}")
    ...
    def test_create_chart_plan_yoy_cascade_electric_vehicles(self):
        """
        Test that create_chart_plan returns correct ChartPlan object for YoY cascade chart of electric vehicles.
        
        This test validates that when given:
        - A DataFrame with year-over-year data (year_report, total_ev_registrations)
        - A user query specifically asking for cascade/waterfall visualization
        
        The function returns a ChartPlan object matching the expected structure for CASCADE chart.
        """
        ...
        # Load expected ChartPlan structure
        expected_plan_path = project_root / "tests" / "test_data" / "chart_plan_yoy_cascade.json"
        with open(expected_plan_path, 'r') as f:
            expected_plan_dict = json.load(f)
        
        # Test query specifically asking for cascade/waterfall
        user_query = "Show me a waterfall chart of year-over-year electric vehicle registration changes from 2023 to 2024?"
        ...
        print(f"✅ Test passed: create_chart_plan returned correct ChartPlan object for CASCADE chart")
        print(f"   Chart Type: {result_plan_dict['chart_type']}")
        print(f"   X Column: {result_plan_dict['x']}")
        print(f"   Y Columns: {result_plan_dict['y']}")
        print(f"   Transform: {result_plan_dict['transform']}")
        print(f"   Title: {result_plan_dict['title']}")
