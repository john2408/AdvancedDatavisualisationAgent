#!/usr/bin/env python3
"""
SQLite Database Creator for Vehicle Registered Data - Traditional Star Schema

This script reads the flat fact_registered_vehicles parquet file and existing dimension tables
to create a SQLite database with a traditional star schema structure optimized for analytical queries.
...
Usage:
    python scripts/create_sqlite_registered_vehicles.py
"""
...
class StarSchemaToSQLite:
    ...
    def __init__(self, star_schema_dir: str = "data/star_schema", 
                 db_path: str = "registered_vehicles.sqlite"):
        """
    ...
    def create_fact_registered_vehicles_table(self) -> None:
        ...
        CREATE TABLE IF NOT EXISTS FactRegisteredVehicles (
            vehicle_count_id TEXT PRIMARY KEY,
    ...
    def drop_all_tables(self) -> None:
        """Drop all tables to ensure clean recreation."""
        logger.info("Dropping existing tables...")
        
        # Disable foreign key constraints temporarily
        self.connection.execute("PRAGMA foreign_keys = OFF")
        
        # Drop tables in reverse order (facts first, then dimensions)
        all_tables = self.fact_tables + list(reversed(self.dimension_tables))
    ...
    def load_table_data(self, table_name: str) -> None:
        ...
        # Insert data into SQLite (tables are already clean)
    ...
    def prepare_fact_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if not df['level_2_district_postcode'].isna().all():
    ...
    def add_unknown_dimension_entries(self) -> None:
        ...
        self.connection.execute("""
            INSERT OR IGNORE INTO DimOEM (oem_key, oem_name, oem_category, country_origin)
            VALUES (-1, 'Unknown', 'Unknown', 'Unknown')
        """)
        
        # Add Unknown Vehicle
        self.connection.execute("""
            INSERT OR IGNORE INTO DimVehicle (vehicle_key, body_type, fuel_type, vehicle_desc)
            VALUES (-1, 'Unknown', 'Unknown', 'Unknown Vehicle Type')
        """)
        
        # Add Unknown Country
        self.connection.execute("""
            INSERT OR IGNORE INTO DimGeographyCountry (geography_country_key, country_name, country_code)
            VALUES (-1, 'Unknown', 'UNK')
        """)
        
        # Add Unknown District
        self.connection.execute("""
            INSERT OR IGNORE INTO DimGeographyDistrict (geography_district_key, country_name, country_code, region_name, district_postcode, district_town_name, full_location_path)
    ...
    def validate_data_integrity(self) -> Dict[str, any]:
        ...
        invalid_counts = self.connection.execute(f"""
    ...
    def generate_summary_report(self, validation_results: Dict, query_results: Dict) -> str:
        ...
        report = []
        report.append("=" * 70)
        report.append("VEHICLE REGISTERED DATA SQLITE DATABASE CREATION SUMMARY")
...
def main():
    ...
    possible_db_paths = [
        os.path.join(project_root, "registered_vehicles.sqlite"),
        os.path.join(project_root, "data", "registered_vehicles.sqlite"),
        "registered_vehicles.sqlite"  # Current directory
    ]
